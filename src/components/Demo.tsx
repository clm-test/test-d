...
// const userFid= context?.user.fid || "268438"
    const handleClaim = () => {
      setIsClicked(true);
      setTimeout(() => {
    ...
    //     args: [userFid], // Only passing fid, since recipient is msg.sender
    //   });
    //   sendTransaction(
    //     {
...
//     dc(String("mmmmmmmm"),String(context?.user.username)); 
...
const casting = async () => {
  ...
  if (hash) {
    setCastHash(hash); // ✅ Now it's available to the component
  }
};
...
//     const [isClicked, setIsClicked] = useState(false);
// const [claimStatus, setClaimStatus] = useState("Minted, Claim now")
  
//     const CLAIM_ADDRESS = "0x348775AC50b487Bb73551b2DEe0842aed2B30922";
//     const MINT_ADDRESS = "0x3DB019427f05192F8FB64534CF9C0bF5cc596a80";
...
<div         className="text-white flex-1 text-center p-2 mb-2 font-medium rounded-xl text-lg shadow-lg relative overflow-hidden transform transition-all duration-200 hover:scale-110 active:scale-95 items-center justify-center hidden"
 style={{
          background: "linear-gradient(90deg, #8B5CF6, #7C3AED, #A78BFA, #8B5CF6)",
          backgroundSize: "300% 100%",
          animation: "gradientAnimation 3s infinite ease-in-out"
        }}
        onClick={()=>sdk.actions.openUrl("https://farcaster.xyz/cashlessman.eth/0xff6f0949")}>
if you like this mini app, Please like and share this cast to support the dev
...
>
  ...
  //         {isConfirming && !isConfirmed && currentAction === "mint"
  //                         ? "Minting..."
  //                         : isConfirmed
  //                         ? "Minted! claim now"
